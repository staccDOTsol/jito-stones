...
use crate::event_loops::{
    pending_tx_loop, slot_subscribe_loop,
};
use futures_util::StreamExt;
use jito_protos::searcher::mempool_subscription;
use solana_client::rpc_client::RpcClient;
use solana_sdk::{commitment_config::CommitmentConfig, signature::{read_keypair_file, Keypair}, vote::error};
use std::str::FromStr;
use {
    crate::{
    ...
    jito_protos::{
        ...
        searcher::{MempoolSubscription, PendingTxNotification, ProgramSubscriptionV0},
    },
    log::{error, info},
    ...
    solana_sdk::{
        ...
        pubkey::Pubkey,
        signature::Signature,
        transaction::SanitizedTransaction,transaction::SanitizedVersionedTransaction
    },
    solana_transaction_status::{Reward, TransactionStatus, TransactionStatusMeta},
    std::{
...
#[derive(Debug, Default)]
pub struct SlotMessages {
   pub messages: Vec<Option<Message>>, // Option is used for accounts with low write_version
   pub  block_meta: Option<MessageBlockMeta>,
   pub  transactions: Vec<MessageTransactionInfo>,
   pub  accounts_dedup: HashMap<Pubkey, (u64, usize)>, // (write_version, message_index)
   pub  entries: Vec<MessageEntry>,
   pub  sealed: bool,
   pub  entries_count: usize,
   pub confirmed_at: Option<usize>,
   pub finalized_at: Option<usize>,
}
...
impl GrpcService {
    ...
    > {
        ...
        let backrun_accounts = vec![
            ...
            "8888882FsxBq7dmc4N1zcMWSKbbsuKgXvfUif2P7S6K6",
        ];
        let backrun_pubkeys: Vec<Pubkey> = backrun_accounts
            ...
            read_keypair_file(Path::new(&"/home/sol/arb.json".to_string())).expect("parse kp file"),
        );

        info!("backrun pubkeys: {:?}", backrun_pubkeys);
        let block_engine_addr = "https://ny.mainnet.block-engine.jito.wtf".to_string();
        tokio::spawn(pending_tx_loop(
            block_engine_addr.clone(),
            auth_keypair.clone(),
            backrun_pubkeys,
            broadcast_tx.clone(),
        ));
